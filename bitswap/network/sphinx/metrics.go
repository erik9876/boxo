...
type JobMetrics struct {
	// registered jobs; failed sends settle through the job result, not
	// through StartJob
	...
	// undecodable and consumed only their envelope, the branch stays open
	// until its last envelope is spent
	RepliesCollected atomic.Uint64
	RepliesFailed    atomic.Uint64
	RepliesDuplicate atomic.Uint64
	RepliesInvalid   atomic.Uint64
	Retransmits      atomic.Uint64
	// no second wave, the job settled on the draw or build error
...
// TransportMetrics counts the pre-dial next-hop lookups. With a finder
// configured, Lookups tracks the SendPacket calls that found no standing
// connection, which is exactly the dial normalization the evaluation
// checks
